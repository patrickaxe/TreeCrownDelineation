...
# FIX PROJ DATABASE PATH - ADD THIS BEFORE OTHER IMPORTS
# Point to the correct PROJ data directory for your Python GDAL installation
import osgeo
proj_data_path = os.path.join(os.path.dirname(osgeo.__file__), 'data', 'proj')
...
# if not hasattr(np, 'float'):
    # np.float = float  # comptibility fix for newer numpy versions where np.float is removed.
...
# MOVED work FUNCTION HERE - OUTSIDE if __name__ == '__main__'
def work(work_args):
    """Worker function for multiprocessing.
    
    Args:
        work_args: Tuple of (feature_enum, dest_fname_base, src_fname, src_xarray_path, 
                             restrict_to_intersection, mask, no_overwrite)
    """
    feature_enum, dest_fname_base, src_fname, restrict, mask_flag, no_overwrite = work_args
    
    # Re-open the xarray in each worker process
    # source_raster = xr.open_rasterio(src_fname, "r", cache=False)
    source_raster = rioxarray.open_rasterio(src_fname, cache=False)
    clip_and_save(feature_enum,
                  dest_fname_base,
                  src_fname,
                  source_raster,
                  None,
                  restrict_to_intersection=restrict,
                  mask=mask_flag,
...
if __name__ == '__main__':
    ...
    for i, file_ in enumerate(input_files):
        ...
        # source_raster = xr.open_rasterio(file_, "r", cache=False)
        source_raster = rioxarray.open_rasterio(file_, cache=False)
        ...
        with Pool(args.nprocs) as p:
            with fiona.open(shapefile) as src:
                ...
                # Prepare work arguments for each feature
                work_args_list = [
                    (feat_enum, dest_fname_base, file_, args.r, args.m, args.no_overwrite)
                    for feat_enum in enumerate(src.filter(bbox=source_raster_bbox.bounds))
                ]
                
                p.map(work, work_args_list, chunksize=args.chunksize)