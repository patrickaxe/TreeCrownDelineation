#!/usr/bin/env python3
import os
os.add_dll_directory(r'C:\Users\Patrick\miniconda3\envs\geo_311\Library\bin')
os.add_dll_directory(r'C:\Users\Patrick\miniconda3\envs\geo_311\Lib\site-packages\osgeo')
import rioxarray
import sys
import fiona.errors
import torch
...
if __name__ == '__main__':
    ...
    array = rioxarray.open_rasterio(args.input_file)
    array = array[:3]  # 只取RGB，去掉Alpha
    ...
    try:
        ...
        # crs_ = get_crs(array)
        crs_ = array.rio.crs.to_epsg()
        crs_ = fiona.crs.from_epsg(crs_)
        ...
        print("Post-processing time: {}s".format(int(postprocessing_time)))
        print("Saving as {}".format(args.output_file))
        crs_ = fiona.crs.from_epsg(array.rio.crs.to_epsg())
        utils.save_polygons(polygons,
                            args.output_file,
                            crs=crs_,
                            driver="GPKG")
